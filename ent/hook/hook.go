...
// The HotBoardFunc type is an adapter to allow the use of ordinary
// function as HotBoard mutator.
type HotBoardFunc func(context.Context, *ent.HotBoardMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HotBoardFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HotBoardMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HotBoardMutation", m)
}
